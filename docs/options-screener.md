...
6. Results start with highest APR first. Click any column heading to toggle sorting. Choose 10, 50, or 100 rows and use First/Previous/Next/Last. Minimums can be changed during or after retrieval without more API calls. Missing APR or probability values do not pass the filters.
...
The local and cloud paths use the same APR-chain calculation code as the Explorer. Put APR uses strike-price collateral; call APR uses current-share-price collateral. Premium is the bid/ask midpoint **per share**. APR is a simple annualized premium-selling measure, not a forecast of option-buying returns. The displayed chance of exercise is `1 − probabilityExpiresWorthless`, using the Explorer's Black–Scholes estimate with zero rates/dividend yield; it estimates expiration exercise, not early assignment.
