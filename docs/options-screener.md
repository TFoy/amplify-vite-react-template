...
On first use, the local page starts with **INTC, AMZN, GOOGL**. Subsequent visits restore its own last saved ticker list, including an intentionally empty list. It does not require an AWS account, Cognito login, Yahoo API key, or `amplify_outputs.json`. Yahoo calls run in Node on your computer, not in the browser. Internet access to Yahoo is required. `npm run dev` and `npm run preview` alone do not start this local API.

## Run using the cloud backend

The hosted page is **/options-screener**, also linked from the landing page. Sign in using the existing account menu. The screener restores its own saved ticker list for that user. On first use it starts empty; add symbols manually or click **Use Options APR explorer tickers** to copy the Explorer's current history/remembered ticker list. This replaces and saves the screener list without modifying the Explorer.
...
Run settings (minimum APR, minimum probability, expiration limit, option types, strike coverage, and outlier exclusion) save automatically as they are edited. Cloud mode saves them in the signed-in user's owner-protected UserPreference record; local mode uses this browser's local storage. New users default to outlier exclusion enabled. The independent screener ticker list is also saved and restored, including its order and an empty list. Explorer tickers are read only when explicitly imported. Retrieved option results are not persisted.
